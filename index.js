...
const pool = new Pool({
    ...
    host: 'localhost',
    database: 'softjobs',
    password: 'Skateforlife123',  // Reemplaza con la contraseña de PostgreSQL
    port: 5432,
});
...
const secretKey = 'holahola123';  // Cambia esta clave secreta por una clave segura
...
app.post('/usuarios', async (req, res) => {
    ...
    try {
        ...
        // Enviar respuesta con el mensaje de bienvenida y el email del usuario registrado
        res.status(201).json({ message: `BIENVENIDO ${newUser.email}` });
    } catch (error) {
        console.error('Error al registrar usuario:', error);
        res.status(500).json({ error: 'Error al registrar usuario' });
    }
